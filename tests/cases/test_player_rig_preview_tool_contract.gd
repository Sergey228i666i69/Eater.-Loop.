...
func run() -> Array[String]:
	...
	assert_true(source.find("POSE_ANIMATIONS = (\"idle\", \"light_idle\", \"walk\", \"light_walk\", \"run\", \"light_run\")") != -1, "Scene-context preview must support all player skeleton motion clips")
	assert_true(source.find("NO_FLASHLIGHT_SCENE_POSES") != -1 and source.find("(\"run contact\", \"run\", 0.1375, False)") != -1, "Scene-context preview must sample no-flashlight run for the no-flashlight sheet")
	assert_true(source.find("FLASHLIGHT_SCENE_POSES") != -1 and source.find("(\"light_idle flashlight\", \"light_idle\", 0.0, True)") != -1, "Scene-context preview must sample light_idle for the flashlight sheet")
	assert_true(source.find("FLASHLIGHT_SCENE_POSES") != -1 and source.find("(\"light_walk contact\", \"light_walk\", 0.2, True)") != -1, "Scene-context preview must sample light_walk for the flashlight sheet")
	assert_true(source.find("pose_animation.startswith(\"light_\")") != -1, "Scene-context dark close-up must auto-show the flashlight for light_* clips")
	...
	assert_true(montage_source.find("\"light_idle\": 1.6") != -1, "Rig preview montage must know the light_idle animation length")
	...
	assert_true(montage_source.find("\"light_idle\", 0.8, True") != -1, "Rig preview runtime set must include flashlight light_idle poses")
