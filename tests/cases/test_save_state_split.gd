...
func run() -> Array[String]:
	...
	assert_eq(int(CycleState.get_phase()), int(CycleState.Phase.DISTORTED), "Cycle phase must restore from CycleState section")
	...
	assert_eq(int(CycleState.get_phase()), int(CycleState.Phase.NORMAL), "Cycle phase must reset on next_cycle")
