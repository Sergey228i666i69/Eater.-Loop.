...
const FRONT_UPPER_ARM_TORSO_UNDERLAP_MIN_RIGHT_X := -20.0
...
const HAND_EDGE_VISIBLE_DARK_FRINGE_SAMPLE_WIDTH := 5
const HAND_EDGE_VISIBLE_DARK_FRINGE_MIN_ALPHA := 0.235
				...
				if visual_texture != null:
					...
					if visual_path == CLEANED_SEAM_FILL_VISUAL_PATH:
						...
						_assert_hand_cutout_does_not_keep_visible_dark_edge_fringe(
								visual_texture,
								visual_path,
								FRONT_HAND_EDGE_MAX_VISIBLE_DARK_FRINGE_PIXELS
						)
...
func _assert_hand_cutout_does_not_keep_visible_dark_edge_fringe(texture: Texture2D, visual_path: String, max_fringe_pixels: int) -> void:
	var image := texture.get_image()
	assert_true(image != null, "Player skeleton hand texture must expose alpha pixels: %s" % visual_path)
	if image == null:
		return
	...
	for y in range(image.get_height()):
		...
		if max_x < 0:
			continue
		for x in range(min_x, mini(image.get_width(), min_x + HAND_EDGE_VISIBLE_DARK_FRINGE_SAMPLE_WIDTH)):
			checked_pixels[Vector2i(x, y)] = true
		for x in range(maxi(0, max_x - HAND_EDGE_VISIBLE_DARK_FRINGE_SAMPLE_WIDTH + 1), max_x + 1):
			checked_pixels[Vector2i(x, y)] = true
	for x in range(image.get_width()):
		var min_y := image.get_height()
		var max_y := -1
		...
		if max_y < 0:
			continue
		for y in range(min_y, mini(image.get_height(), min_y + HAND_EDGE_VISIBLE_DARK_FRINGE_SAMPLE_WIDTH)):
			checked_pixels[Vector2i(x, y)] = true
		for y in range(maxi(0, max_y - HAND_EDGE_VISIBLE_DARK_FRINGE_SAMPLE_WIDTH + 1), max_y + 1):
			checked_pixels[Vector2i(x, y)] = true
	var visible_dark_fringe_pixels := 0
	for point in checked_pixels.keys():
		if _is_visible_dark_hand_edge_fringe_pixel(image.get_pixel(point.x, point.y)):
			visible_dark_fringe_pixels += 1
	assert_true(
			visible_dark_fringe_pixels <= max_fringe_pixels,
			"Player skeleton hand cutout edge must not keep visible dark source-matte fringe pixels: %s" % visual_path
	)

func _is_visible_dark_hand_edge_fringe_pixel(color: Color) -> bool:
	if color.a <= HAND_EDGE_VISIBLE_DARK_FRINGE_MIN_ALPHA:
		return false
	var luminance := 0.2126 * color.r + 0.7152 * color.g + 0.0722 * color.b
