...
const DEFAULT_BG: Texture2D = preload("res://levels/minigames/feeding/BackgroundBasic.png")
...
func setup_game(andrey_texture: Texture2D, count: int, music: AudioStream, win_sound: AudioStream, eat_sound_override: AudioStream = null, bg_override: Texture2D = null, food_scenes: Array[PackedScene] = []) -> void:
	...
	if tarelka_scene:
		...
		push_error("Не удалось найти сцену тарелки по пути res://levels/minigames/feeding/food/plate/plate.tscn")
...
func _close_game() -> void:
	if MinigameController:
		MinigameController.finish_minigame_with_fade(self, true, Callable(self, "_emit_minigame_finished"))
	else:
		minigame_finished.emit()

func _emit_minigame_finished() -> void:
	minigame_finished.emit()
