...
func setup(music_manager: Object, stream: AudioStream, volume_db: float, suspend_music: bool) -> void:
	...
	_music_pushed = true
	if stream != null:
		music_manager.call("start_minigame_music", stream, volume_db)
		return
	music_manager.call("push_music", null, _music_fade_time)


func restore(music_manager: Object) -> void:
	if music_manager == null:
		return
	if not _music_pushed:
		return
	if _music_is_stream:
		music_manager.call("pop_music", 0.0)
		_clear_pushed_state()
		return
	if _music_stop_on_finish:
		music_manager.call("stop_music", _music_fade_time)
	music_manager.call("pop_music", _music_fade_time)
	_clear_pushed_state()


func stop(music_manager: Object, fade_time: float = -1.0) -> void:
	if music_manager == null:
		return
	if not _music_pushed:
		return
	if _music_is_stream:
		...
		return
	music_manager.call("stop_music", _resolve_fade_time(fade_time))
...
func update(
	...
	if not _music_pushed:
		...
		music_manager.call("start_minigame_music", stream, volume_db)
		return
	var mixed_volume := float(music_manager.call("resolve_mix_volume_db", mix_name, volume_db))
	music_manager.call("play_music", stream, target_fade, mixed_volume, 0.0, 999.0, source_id, source_kind)
