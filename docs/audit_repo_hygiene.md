...
- `MinigameController`: около 589 строк + `minigame_backdrop_presenter.gd` + `minigame_prompt_visibility_coordinator.gd` + `minigame_timer_state.gd` + `minigame_modal_ownership.gd` + `minigame_music_session.gd` + `gamepad_scheme_registry.gd`; `GamepadRuntime`: около 577 строк + `gamepad_hint_builder.gd`;
