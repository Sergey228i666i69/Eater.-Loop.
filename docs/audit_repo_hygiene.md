...
- `GameDirector`: было 1171 строка, стало около 865 строк + `game_director_cycle_phase_bridge.gd` + `game_director_cycle_timer_state.gd` + `game_director_death_title_presenter.gd` + `game_director_death_camera_coordinator.gd` + `game_director_death_cursor_coordinator.gd` + `game_director_death_fade_coordinator.gd` + `game_director_death_retry_coordinator.gd` + `game_director_death_screen_reset.gd` + `game_director_death_sequence_state.gd` + `game_director_distortion_gate.gd` + `game_director_distortion_overlay_coordinator.gd` + `game_director_distortion_phase_state.gd` + `game_director_distortion_progress.gd` + `game_director_timer_node_coordinator.gd` + `game_director_stalker_service.gd` + `game_director_overlay_layer_coordinator.gd`;
