...
- `GameDirector`: было 1171 строка, стало около 876 строк + `game_director_cycle_phase_bridge.gd` + `game_director_cycle_timer_state.gd` + `game_director_death_title_presenter.gd` + `game_director_death_entry_presenter.gd` + `game_director_death_camera_coordinator.gd` + `game_director_death_cursor_coordinator.gd` + `game_director_death_fade_coordinator.gd` + `game_director_death_retry_coordinator.gd` + `game_director_death_screen_reset.gd` + `game_director_death_sequence_state.gd` + `game_director_distortion_gate.gd` + `game_director_distortion_overlay_coordinator.gd` + `game_director_distortion_phase_state.gd` + `game_director_distortion_progress.gd` + `game_director_timer_node_coordinator.gd` + `game_director_stalker_service.gd` + `game_director_overlay_layer_coordinator.gd`;
- `GameState`: около 334 строк, а scene checkpoint snapshot/restore уже живёт в `checkpoint_scene_snapshot.gd` + `checkpoint_dynamic_restore.gd`;
- `MusicManager`: около 1133 строк, публичный facade оставлен стабильным;
- `MinigameController`: около 589 строк + `minigame_backdrop_presenter.gd` + `minigame_prompt_visibility_coordinator.gd` + `minigame_timer_state.gd` + `minigame_modal_ownership.gd` + `minigame_music_session.gd` + `gamepad_scheme_registry.gd`; `GamepadRuntime`: около 469 строк + `gamepad_hint_builder.gd` + `gamepad_navigation_repeat.gd` + `gamepad_node_resolver.gd` + `gamepad_callback_router.gd` + `gamepad_confirm_release_gate.gd`;
- `Player`: остаётся фасадом движения/анимации/инвентаря, но state helpers уже вынесли `player_stamina_state.gd`, `player_facing_state.gd`, `player_inventory_state.gd`, `player_skeleton_step_state.gd` и `player_flashlight_charge_state.gd`;
...
Не каждый большой файл плох сам по себе, но здесь размеры совпадали со смешением обязанностей. Безопасные pass-ы вынесли fade/tween state из `UIMessage`, backdrop registry/presentation, prompt suspend/restore lifecycle, timer state, pause/cursor ownership, minigame music stack lifecycle и gamepad scheme registry из `MinigameController`, gamepad hint policy, navigation repeat state, node/provider resolving, callback routing и confirm release gate state из `GamepadRuntime`, scene checkpoint snapshot/restore и dynamic runtime restore policy из `GameState`, pause-reason bookkeeping, ambient suppression source tracking, scoped event/distortion source registry и chase source registry из `MusicManager`, run/stamina state, facing/checkpoint normalization, key inventory/checkpoint state, skeleton step timing state и flashlight charge/recharge state из `Player`, death-title/glitch presentation, death entry presentation setup, stalker spawn/checkpoint service, overlay layer policy, death cursor/input policy, death camera capture/restore, death fade/tween setup, death retry restore/darken/reload flow, death screen reset/cleanup, death sequence active/pause gate state, cycle timer/checkpoint state, CycleState phase bridge, timer node lifecycle, minigame distortion gate, distortion progress/easing math, distortion overlay/material actuator и distortion phase/checkpoint state из `GameDirector`. Более широкий распил `MusicManager`/`GameDirector` остаётся future refactor-ом, а не hygiene-блокером текущего состояния.
