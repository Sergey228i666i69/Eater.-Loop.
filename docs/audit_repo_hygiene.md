...
- `GameDirector`: было 1171 строка, стало около 837 строк + `game_director_cycle_phase_bridge.gd` + `game_director_cycle_timer_state.gd` + `game_director_death_title_presenter.gd` + `game_director_death_camera_coordinator.gd` + `game_director_death_cursor_coordinator.gd` + `game_director_death_retry_coordinator.gd` + `game_director_distortion_gate.gd` + `game_director_distortion_overlay_coordinator.gd` + `game_director_distortion_phase_state.gd` + `game_director_distortion_progress.gd` + `game_director_stalker_service.gd` + `game_director_overlay_layer_coordinator.gd`;
...
Не каждый большой файл плох сам по себе, но здесь размеры совпадали со смешением обязанностей. Безопасные pass-ы вынесли fade/tween state из `UIMessage`, backdrop registry/presentation из `MinigameController`, death-title/glitch presentation, stalker spawn/checkpoint service, overlay layer policy, death cursor/input policy, death camera capture/restore, death retry restore/darken policy, cycle timer/checkpoint state, CycleState phase bridge, minigame distortion gate, distortion progress/easing math, distortion overlay/material actuator и distortion phase/checkpoint state из `GameDirector`. Более широкий распил `MusicManager`/`GameDirector` остаётся future refactor-ом, а не hygiene-блокером текущего состояния.
