...
4. **Полную приватизацию CycleState backing fields всё ещё стоит закрывать отдельной задачей.** RU player-facing localization key coverage уже закреплён тестом; более широкий non-Russian/technical UI coverage можно расширять отдельно.
