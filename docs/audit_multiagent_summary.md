...
| Tooling/assets/tests | 4/10 | LFS/assets и CI починены, export smoke автоматизирован после тестов |
...
2. **Оставшиеся крупные фасады требуют отдельного refactor budget.** `MusicManager`, `GameDirector` и `Player` всё ещё большие; `UIMessage`, `MinigameController` backdrop/prompt/timer/gamepad registry/gamepad hint/gamepad repeat/gamepad node resolving/modal ownership/music session lifecycle, ambient suppression/source lifecycle части `MusicManager`, run/stamina, facing/checkpoint, key inventory, skeleton step timing и flashlight charge/recharge части `Player`, death-title/death-entry/death-fade/death-sequence/stalker/overlay-layer/death-cursor/death-camera/death-retry/death-screen-reset/cycle-timer/cycle-phase/timer-node/distortion-gate/distortion-progress/distortion-overlay/distortion-phase части `GameDirector` уже получили безопасные helper split-ы.
3. **Release artifacts всё ещё требуют отдельного release-процесса.** CI теперь делает MacOS debug export smoke после тестов, но signed/notarized distributable остаётся локальной/release ответственностью.
