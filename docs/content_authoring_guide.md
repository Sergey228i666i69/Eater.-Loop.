...
17. Если root scene script экспортирует `fridge_path`, этот путь считается обязательным и должен резолвиться в `Fridge`; для optional холодильников используй явно именованные поля вроде `primary_fridge_path` и проверяй их отдельным focused contract.
