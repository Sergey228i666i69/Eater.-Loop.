...
3. Русскоязычные player-facing строки в `text`, `prompt_text`, message export-полях, death/ending UI, note/obstacle prompts, lab dialogue exports, money reward reasons, gamepad `hints`, default gamepad hints, `UIMessage.show_*("...")` и `tr("...")` должны иметь ключ в CSV; это проверяется тестом. Не добавляй транслит-ключи вроде ASCII-фраз с пробелами для русских строк: используй русский source text или semantic id. Технические строки без кириллицы вроде таймеров, SQL-заголовков и кнопочных цифр пока не блокируются.
