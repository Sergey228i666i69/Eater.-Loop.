...
7. Настройки бега (`allow_running`, `stamina_*`, `run_speed_multiplier`) задавай через existing player export-поля сцены; drain/recovery/checkpoint semantics живут в `PlayerStaminaState` и покрыты тестом.
