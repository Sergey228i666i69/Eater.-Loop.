...
2. ~~Проверять groups/methods вроде `reactive_light_source`, `turn_on`, `is_point_lit`.~~ Runtime scripts регистрируются через `ReactiveLightContracts`, а validators требуют нужные методы для reactive/generator light contracts.
3. ~~Проверять missing/broken NodePath.~~ Закрыто для active content scene exported non-empty `NodePath`/`Array[NodePath]` values, utility-level paths, root level paths and STU exported route/wiring paths.
4. ~~Проверять checkpoint participants: capture/apply, stable id, dynamic restore.~~ Базовый и dynamic restore покрыты; content scripts с custom checkpoint API обязаны объявлять capture/apply парой, active scene checkpoint participants обязаны иметь стабильный scene-relative path без generated `@...` segments, scene snapshot/restore вынесен в `checkpoint_scene_snapshot.gd`, а dynamic runtime restore policy вынесена в `checkpoint_dynamic_restore.gd` с focused unit-тестом.
...
7. ~~Проверять configured trigger target/property/effect wiring.~~ `test_trigger_set_property_contracts.gd` валидирует `TriggerSetProperty`/`PropertyChange` target paths, property names, наличие хотя бы одного property/sfx/music effect и `music_stream` для replace/event-start music actions.
8. ~~Проверять key-door/search-key wiring.~~ `test_scene_dependency_contracts.gd` требует источник ключа для `required_key_id` дверей и resolving `SearchKeyManager.search_spots`; `test_search_key_authoring_contracts.gd` валидирует managed `SearchSpot` minigame/key/trash configs.
9. ~~Проверять cycle-level authoring metadata и sleep transitions.~~ `test_level_authoring_contracts.gd` валидирует playable `level_*.tscn` root contract, metadata, ровно один Player instance, Player export ranges, bed `next_level_path`, target scene type cycle-level/ending, root exported paths, LevelMusic configs и включённые стартовые тексты.
...
11. ~~Проверять feeding/code-lock/final fridge configs.~~ `test_fridge_authoring_contracts.gd` валидирует typed `FeedingMinigame` scenes, typed `FinalFeedMinigame` scene, food scenes, face/background и code-lock scene; `level_13_STU_3` cafeteria fridge получил полный feeding config.
12. ~~Расширить localization coverage на новые player-facing export-поля при активной работе с UI.~~ Death/ending UI, note/obstacle prompts, timed lab dialogue exports, key names, money reward reasons, gamepad hints и default gamepad hints теперь входят в `test_localization_contracts.gd`; статические `.tscn` строки и прямые GDScript call-literals в `UIMessage.show_*("...")` / `tr("...")` без кириллицы требуют CSV-key или явной technical exception.
...
3. `MusicManager`:
   ...
   - ~~вынести source lifecycle bookkeeping отдельными helper-ами.~~ `MusicAmbientCoordinator`/`MusicAmbientSuppressionState` владеют ambient suppression sources и pending resume request, `MusicScopedSourceRegistry` владеет event/distortion source metadata и `tree_exited` wiring, а `MusicChaseSourceRegistry` владеет chase source order/suppression/active-source metadata и cleanup при выходе source-ноды из дерева.
...
5. `Fridge`:
   ...
   - ~~отделить feeding minigame setup.~~ `FridgeFeedingSession` проверяет config, создаёт typed `FeedingMinigame` node и вызывает `setup_game`.
