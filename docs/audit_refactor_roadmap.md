...
12. ~~Расширить localization coverage на новые player-facing export-поля при активной работе с UI.~~ Death/ending UI, note/obstacle prompts, timed lab dialogue exports, key names и money reward reasons теперь входят в `test_localization_contracts.gd`; технические строки без кириллицы по-прежнему не блокируются.
...
4. `MinigameController`:
   ...
   - music/gamepad runtime lifecycle можно дробить отдельными tested slices.
5. `Fridge`:
