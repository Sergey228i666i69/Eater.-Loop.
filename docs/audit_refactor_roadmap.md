...
9. ~~Проверять cycle-level authoring metadata и sleep transitions.~~ `test_level_authoring_contracts.gd` валидирует metadata, bed `next_level_path`, target scene type cycle-level/ending, root exported paths и включённые стартовые тексты.
