...
- После ремонтных проходов parser-only и полный suite проходят; текущий полный suite содержит 95 тестов.
...
Текущий expected result: `bash tests/run_tests.sh` завершается `OK: all tests passed (95)`.
