...
- После ремонтных проходов parser-only и полный suite проходят; текущий полный suite содержит 71 тест.
...
Текущий expected result: `bash tests/run_tests.sh` завершается `OK: all tests passed (71)`.
