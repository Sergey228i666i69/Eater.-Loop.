...
Реальные цепочки мигрированы явно: laptop→fridge в `level_03_deepseek` и `level_05_sql` используют `INTERACTION_REQUESTED`, `NoteStory`→fridge в `level_11_STU_1` и fridge→generator в `level_12_STU_2` используют `COMPLETED`. Финальная laptop-ветка слушает только typed `interaction_succeeded`, а final scene wiring валидирует реальные `Laptop`/`Fridge`/`Bed` paths и `bad_ending_scene`. Legacy `interaction_finished` остаётся только совместимым success wrapper для старого кода, но не выбирает финальную ветку. Сцены с `dependency_object` теперь валидируются на явный `dependency_condition`, а level scripts не должны вызывать `set_dependency_object()` без близкого `set_dependency_condition()`.
