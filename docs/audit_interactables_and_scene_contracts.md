...
Реальные цепочки мигрированы явно: laptop→fridge в `level_03_deepseek` и `level_05_sql` используют `INTERACTION_REQUESTED`, `NoteStory`→fridge в `level_11_STU_1` и fridge→generator в `level_12_STU_2` используют `COMPLETED`. Финальная laptop-ветка, fridge-driven level redirects и node-signal defaults `TargetMonsterSpawner` слушают typed `interaction_succeeded`, а final scene wiring валидирует реальные `Laptop`/`Fridge`/`Bed` paths и `bad_ending_scene`. Legacy `interaction_finished` остаётся только совместимым success wrapper для старого кода, но не должен использоваться runtime scripts или scene authoring subscriptions. Сцены с `dependency_object` теперь валидируются на явный `dependency_condition`, а level scripts не должны вызывать `set_dependency_object()` без близкого `set_dependency_condition()`.

Два прежних опасных класса багов тоже закрыты. Конкретный softlock из `level_04_findkey`, где `SearchSpot` с `door_key` зависел от двери `ToBedroom`, которая сама требовала `door_key`, закрыт: search spots больше не завязаны на эту дверь, а `test_scene_dependency_contracts.gd` ловит такие key-door циклы. Тот же contract теперь проверяет, что `required_key_id` у дверей имеет источник ключа в той же сцене, а `SearchKeyManager.search_spots` непустой и resolving в typed `SearchSpot`. `test_search_key_authoring_contracts.gd` дополнительно проверяет managed `SearchSpot` typed `SearchKeyMinigame`/key/trash config и запрещает manager/spot runtime возвращаться к stringly method probes для `set_has_key`, `set_searched_empty`, `setup`, `get_layout_state` и `mark_all_spots_searched_empty`. `SearchKeyManager` слушает successful outcome своих spot-ов и сам закрывает sibling spots, чтобы точка поиска не открывала пустую или неполную мини-игру. Это уже поймало и закрыло два реальных scene-долга: несуществующий `lebedka_key` в `level_09_crazy` и locked `Door(In604)` с несуществующим `key_6level` в `level_12_STU_2`.
...
Статус: закрыто на уровне runtime contracts. `test_scene_nodepath_contracts.gd` проверяет active content scene exported non-empty `NodePath`/`Array[NodePath]` values, unlocked/key door targets, blockpost child contracts, typed `Level12MoneySystem` paths, light/sprite/audio exported paths, scene-owned `AudioStreamPlayer`/`AudioStreamPlayer2D` buses, teleport targets, utility-level paths лебёдки/corridor distortion и `TargetMonsterSpawner` condition paths. Для лебёдки `fridge_path` теперь обязан резолвиться именно в `Fridge`, а сам скрипт вызывает `apply_winch_release_state()` типизированно. `test_level_authoring_contracts.gd` валидирует root-level exported `*_path` wiring, ожидаемые типы по имени (`Door`, `Fridge`, `Laptop`, `Bed`, `Player`, `InteractiveObject`) и `LevelMusic` stream/fade config. `test_stu_level_path_contracts.gd` отдельно фиксирует STU floor/room paths, exported route/wiring paths и dynamic redirect targets; level scripts теперь типизируют обязательные door targets как `Door`, а fridge route/wiring paths как `Fridge`, вместо stringly/generic `has_method/call` glue. `test_interaction_architecture_contracts.gd` дополнительно запрещает content-object scripts возвращать `UIMessage.fade_*`, lebedka/fridge wiring и level-12 money API к stringly `has_method/call` probes. `test_trigger_set_property_contracts.gd` проверяет, что configured `TriggerSetProperty`/`PropertyChange` target paths резолвятся, target nodes реально имеют указанное property, каждый trigger имеет хотя бы один property/sfx/music effect, а replace/event-start music actions не остаются без `music_stream`. Inherited door/interactable defaults больше не должны сериализоваться как `null` для typed config-полей: `is_locked`, `required_key_id`, `interact_area_node`, `target_marker`, `one_shot` и соседние defaults должны быть явными.
...
Статус: закрыто. `TargetMonsterSpawner` получил `condition_configured`, реальный инстанс в `level_11_STU_1` явно задаёт `ate_this_cycle == true`, а `test_scene_dependency_contracts.gd` запрещает spawner-ы с `enemy_scene`, которые не подтвердили условие. `test_scene_nodepath_contracts.gd` дополнительно проверяет node-signal и trigger-enter condition paths, signal/property names и optional spawn parent. State-флаги spawner теперь читаются прямыми публичными методами `GameState`/`CycleState` без `state_owner.call(method_name)`, а `test_target_spawner_state_conditions.gd` покрывает все поддержанные флаги.
