...
- `ReactiveLightContracts` (`res://global/reactive_light_contracts.gd`)
  Canonical groups, registration helpers/getters, point-lit query helper and generator activation helper
  для reactive light sources и generator-required light contracts.
...
- `Bed.block_sleep` отвечает за явно запрещённые/декоративные кровати, а `require_light_for_sleep` только за настоящий bedroom-light contract с typed `Lamp.is_bedroom`; runtime не должен возвращаться к `has_method("is_light_active")`.
...
- Тип сцены определяется через `SceneContext`: gameplay, menu и ending имеют отдельные группы/path-классификацию; gameplay также определяется по typed `CycleLevel` root contract, path fallback разрешает только playable `res://levels/cycles/level_*.tscn`, а utility-сцены в cycles должны полагаться на группы/API, если им нужна специальная классификация.
...
- `PauseManager.pause_menu_scene` должен инстанцировать layer с дочерним `PauseMenu` на `levels/menu/pause_menu.gd`; manager вызывает menu lifecycle API типизированно, без `_pause_menu.has_method(...)` или `_pause_menu.call(...)`.
- `GameDirector` death screen тоже использует `PauseManager` token facade напрямую и не должен сам выставлять `get_tree().paused`; overlay layer читает `PauseManager`/`MinigameController` через стабильные facade-вызовы.
...
- Feeding minigames держат drag lifecycle и stage-interaction gating в локальном `FoodItem` state и используют стабильные `MusicManager`/`UIMessage` facades напрямую; мёртвые compatibility hooks вроде `GameState.reset_dragging` и локальные `FoodItem.set_interaction_enabled` method probes не должны возвращаться.
...
- `TimedLabMinigameBase` показывает success/failure outcome dialogue через стабильный `UIMessage.show_dialogue(...)` facade и отмечает lab completion через `CycleState.mark_lab_completed(...)` без stringly method probes.
...
### 2.8 Контур анимации героя (Покадровая анимация — основная; Скелетный риг — неудачный эксперимент)

- `player/player.tscn` является **основной production-сценой игрока** с покадровой анимацией (`AnimatedSprite2D` на кадрах `res://player/animations/walking/ezgif-frame-001.png` .. `016.png` и текстуре простоя `AndryWithFlashlight.png`).
- Все игровые уровни (`res://levels/cycles/*.tscn`) и шаблоны инстансируют `res://player/player.tscn` и используют покадровую спрайтовую анимацию.
- **Эксперимент со скелетным ригом** (`res://player/player_skeleton_rig.tscn`, `res://player/player_skeleton_forearm_mesh_prototype.tscn`) признан **неудачным экспериментом** и выведен из рантайма уровней в архив/прототип.
- Скелетный риг (`PlayerSkeletonRig`) сохранён как изолированный прототип со своими тестами и инструментами предпросмотра (`tools/player_rig_preview/`), но не монтируется в основную сцену игрока `player.tscn`.
- Шаги игрока в `player.tscn` привязаны к номерам кадров покадровой анимации через `StepAudioComponent`.
- `player/LEGASY-ANIMATIONS-CHARACTER.tscn` сохранён как историческая копия спрайтовой сцены, синхронизированная с `player.tscn`.
...
- Все игровые уровни продолжают ссылаться на `res://player/player.tscn`, поэтому автоматически используют основной покадровый вариант персонажа.
- Playable scenes в `res://levels/cycles/` должны называться `level_*.tscn` и иметь root script `CycleLevel`; non-level utility-сцены в этой папке требуют явного allowlist в `test_level_authoring_contracts.gd`.
...
- `GameDirectorCyclePhaseBridge` тоже ходит к phase API `CycleState` напрямую; `null` остаётся явным fallback-ом для отсутствующего autoload, но "похожий" объект без phase API не должен тихо маскировать сломанный контракт.
...
- Content objects with singleton-owned state/flow (`TargetMonsterSpawner`, `PickupFlashlight`, `final_ending_fridge`, `Laptop`, `Lebedka`) call stable `CycleState`/`GameState`/`GameDirector` facades directly; local typed helpers like flashlight pickup cleanup and spawner state-flag checks also avoid `has_method/call` probes. Group-local polymorphism can still use explicit `has_method` where nodes are intentionally heterogeneous.
- Local UI helpers such as `PromptView` should prefer stable node properties (`text`) over probing for optional setter methods.
- Player-dependent content/HUD/enemy detection (`Door`, key/note rewards, search-key minigame, `Lebedka`, stamina/flashlight bars, flashlight-sensitive enemies) also treats the scene `player` group as a stable `Player` facade: key inventory, physics toggles, stamina and flashlight state are called directly after a `null` guard.
...
- Death entry/fade/retry/cursor helpers also treat their collaborators (`GameState`, `CycleState`, `UIMessage`, `CursorManager`, death title/camera helpers) as stable facades. They keep `null` guards for genuinely optional collaborators, but do not keep method-probe fallbacks for half-present APIs.
...
- Основной персонаж игрока (`player/player.tscn`) и все игровые уровни возвращены на каноничную покадровую спрайтовую анимацию (`AnimatedSprite2D`); скелетный риг (`player_skeleton_rig.tscn`) признан неудачным экспериментом и сохранён исключительно в виде изолированного архивного прототипа.
