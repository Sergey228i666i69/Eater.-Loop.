...
- Следующее направление улучшения качества рига - `Polygon2D`/weighted mesh-деформация отдельных частей вместо бесконечного склеивания Sprite2D-cutout-ов. Прототип хранится отдельно в `res://player/player_skeleton_forearm_mesh_prototype.tscn`: он использует текущий `front_forearm.png`, внутренние vertices и веса на `FrontUpperArm`/`FrontForearm`, но не подключён к активному `player.tscn`, пока не будет доказано, что mesh даёт лучший силуэт в игровых превью.
