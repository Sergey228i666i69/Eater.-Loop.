...
- Внутренние helper-и крупных фасадов:
  `GameDirectorDeathCameraCoordinator`, `GameDirectorDeathCursorCoordinator`, `GameDirectorDeathTitlePresenter`, `GameDirectorDistortionGate`, `GameDirectorOverlayLayerCoordinator`, `GameDirectorStalkerService`, `MinigameBackdropPresenter`, `UIFadeController`.
...
- `UIMessage`, `MinigameController`, death-title часть, stalker spawn/checkpoint часть, overlay layer policy, death cursor/input policy, death camera capture/restore и minigame distortion gate `GameDirector` получили facade-preserving helper split-ы.
