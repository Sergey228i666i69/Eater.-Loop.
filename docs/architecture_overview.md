...
- Добавлены scene-contract validators для critical NodePath/child contracts, utility-level NodePaths, cycle-level authoring contracts, configured trigger target/property wiring, key-door/search-key wiring и отдельные STU path contracts.
