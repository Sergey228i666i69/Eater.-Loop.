...
- `MeshFrontUpperArm` правым контуром underlap-ит torso-слой до x >= -20: кожа верхней руки заполняет прозрачные пиксели у подмышки/бока, но сама футболка остаётся выше по `z_index` и закрывает шов.
