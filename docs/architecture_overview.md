...
- `MeshFrontUpperArm` правым контуром taper-underlap-ит torso-слой: верх/середина заходят глубже под футболку до x >= -2, а нижний край остаётся сдержанным на x >= -14. Кожа верхней руки заполняет прозрачные пиксели у подмышки/бока, но сама футболка остаётся выше по `z_index` и закрывает шов.
