...
- Content objects with singleton-owned state/flow (`pickup_flashlight`, `final_ending_fridge`) call stable `CycleState`/`GameState`/`GameDirector` facades directly; group-local polymorphism can still use explicit `has_method` where nodes are intentionally heterogeneous.
