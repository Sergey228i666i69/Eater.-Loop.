...
- `MeshFrontUpperArm` правым контуром немного underlap-ит torso-слой: кожа верхней руки заполняет прозрачные пиксели у подмышки/бока, но сама футболка остаётся выше по `z_index` и закрывает шов.
