...
- Добавлен компактный `VisualNeckCollarCover` на chest-side seam: он рендерится поверх torso, но ниже rotating head, чтобы при head/neck sway не появлялся тёмный провал между головой и воротом.
