...
- Размах `walk`/`light_run` намеренно сдержанный: риг построен из фото-cutout частей, поэтому тесты держат нижнюю и верхнюю границу limb-swing и foot-lift, чтобы движение было заметным, но не разрывало суставы.
