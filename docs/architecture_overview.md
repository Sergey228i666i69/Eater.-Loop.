...
- Добавлены scene-contract validators для critical NodePath/child contracts, utility-level NodePaths, cycle/lab/fridge-level authoring contracts, configured trigger target/property wiring, key-door/search-key wiring, checkpoint participant stable paths и отдельные STU path contracts.
