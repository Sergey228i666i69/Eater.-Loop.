...
- Внутренние helper-и крупных фасадов:
  `GameDirectorDeathCameraCoordinator`, `GameDirectorDeathCursorCoordinator`, `GameDirectorDeathTitlePresenter`, `GameDirectorOverlayLayerCoordinator`, `GameDirectorStalkerService`, `MinigameBackdropPresenter`, `UIFadeController`.
...
- `UIMessage`, `MinigameController`, death-title часть, stalker spawn/checkpoint часть, overlay layer policy, death cursor/input policy и death camera capture/restore `GameDirector` получили facade-preserving helper split-ы.
