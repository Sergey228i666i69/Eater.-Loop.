...
- Внутренние helper-и крупных фасадов:
  `GameDirectorDeathTitlePresenter`, `GameDirectorStalkerService`, `MinigameBackdropPresenter`, `UIFadeController`.
...
- `UIMessage`, `MinigameController`, death-title часть и stalker spawn/checkpoint часть `GameDirector` получили facade-preserving helper split-ы.
