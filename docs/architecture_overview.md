...
- `SkeletonAnimationPlayer` внутри рига держит loop-клипы `idle`, `walk`, `run` и `light_run` с cubic-интерполяцией bone-tracks; `idle` даёт плавное дыхание корпуса, головы и кистей, а `player.gd` переключает клипы по фактическому движению и наличию фонарика.
- Размах `walk`/`run`/`light_run` держится в контролируемом коридоре: риг построен из фото-cutout частей, поэтому тесты держат нижнюю и верхнюю границу body-bounce, spine/chest/neck/head counter-sway, leg-swing, foot-lift, foot-roll, foot-stride, arm/forearm/wrist follow-through и flashlight bob. У дальней ноги намеренно сдержанный thigh/shin swing, чтобы фото-фрагменты штанин не выезжали прямоугольными плитками; `idle` тоже даёт мягкий bob фонарика вместе с дыханием руки, `run` для no-flashlight состояния свободнее работает передней рукой и пустой кистью, а `light_run` удерживает фонарик с отдельным bob, но не меняет contact-time шагов.
...
- Фонарик является отдельным cutout-слоем из `AndryWithFlashlight.png` на `FlashlightMount`; его ручка дорисована как цельный скрытый под ладонью сегмент, а слой рендерится под передней held-кистью, чтобы рука перекрывала середину фонарика как на исходнике. `VisualFlashlight` посажен чуть выше и правее старого offset, потому что внутри texture есть прозрачный верх; видимая ручка должна проходить через зону хвата, а не висеть ниже пальцев. Основной skeleton общий, но `player.gd` выбирает `run` до разблокировки фонарика и `light_run` после неё; `FrontHand` держит два совместимых cutout-а кисти: `VisualFrontHandEmpty` из `Andry.png` для no-flashlight состояния и `VisualFrontHand` из `AndryWithFlashlight.png` для хвата фонарика. Пустая кисть дополнительно получает локальный wrist-sway на своём `Sprite2D`, чтобы no-flashlight вариант не выглядел как зафиксированный хват фонарика.
- Skeleton-only звуки шагов привязаны к contact-time внутри `walk`/`run`/`light_run`, а не к независимому таймеру; тест рига сверяет эти таймкоды с нижним краем alpha-пикселей стоп. Foot-треки `walk`/`run`/`light_run` держат промежуточные passing-ключи между контактами, чтобы стопа шла дугой без резкого рывка, но сами contact-time точки остаются прежними.
