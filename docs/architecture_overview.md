...
- Для передней руки используется гибридный порядок слоёв: sprite `VisualFrontUpperArm` остаётся ниже корпуса по z-index, чтобы torso закрывал фото-шов плеча, а sprite `VisualFrontForearm` и кисть `VisualFrontHand` остаются над корпусом вместе с фонариком. Верх `front_forearm.png` должен fade-иться под верхнюю руку, иначе при ходьбе он читается как жёстко наклеенный прямоугольный elbow-crop.
...
- Активные runtime-конечности возвращены с `Polygon2D` на `Sprite2D`-слои, привязанные к костям: нативный Godot-render снова показывает плечи, предплечья, бёдра и голени, а mesh-deformation остаётся только в отдельном прототипе до доказанной проверки в самом Godot.
