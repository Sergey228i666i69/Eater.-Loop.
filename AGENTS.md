...
2. **Полный тестовый прогон зелёный на момент последней проверки.** `bash tests/run_tests.sh` проходил с 95 тестами без `ObjectDB instances leaked at exit`.
...
- Есть локальный тест-раннер и 95 тестов.
...
На момент последней проверки parser-only проходил, а полный suite проходил с 95 тестами. Перед релизными выводами или крупным рефакторингом нужно перепроверить текущее состояние командой выше.
