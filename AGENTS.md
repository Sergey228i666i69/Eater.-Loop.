...
2. **Полный тестовый прогон зелёный на момент последней проверки.** `bash tests/run_tests.sh` проходил с 84 тестами без `ObjectDB instances leaked at exit`.
...
- Есть локальный тест-раннер и 84 теста.
- Тесты уже проверяют autoload-и, main scene, project config, localization CSV/mojibake hygiene, RU player-facing localization key coverage, cycle/lab authoring contracts, загрузку сцен и запрет использования приватного API `MusicManager`.
...
На момент последней проверки parser-only проходил, а полный suite проходил с 84 тестами. Перед релизными выводами или крупным рефакторингом нужно перепроверить текущее состояние командой выше.
