...
6. **Dependency-система интерактивов стала безопаснее, но всё ещё ломкая.** Конкретный цикл `SearchSpot -> Door(ToBedroom) -> door_key` в `level_04_findkey` закрыт тестом, но зависимости всё ещё смотрят на прямой `is_completed`.
