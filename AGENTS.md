...
- Тесты уже проверяют autoload-и, main scene, project config, localization CSV/mojibake/translit-key/static non-Cyrillic text hygiene, RU player-facing localization key coverage including gamepad hints, cycle/lab/fridge authoring contracts, загрузку сцен и запрет использования приватного API `MusicManager`.
...
3. Расширять localization validator на GDScript non-Cyrillic UI literals, если эта зона начнёт активно меняться.
