...
- `tests/cases/test_fridge_authoring_contracts.gd` валидирует active level feeding/code-lock/final fridge configs, включая typed `FeedingMinigame` scenes, typed `FinalFeedMinigame` scene, typed `FoodItem` food scenes, face/background и code-lock scene; final-stage food interaction gating теперь тоже идёт через typed `FoodItem`, без `set_interaction_enabled` method probes.
