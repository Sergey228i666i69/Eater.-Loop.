...
- `MusicManager` facade оставлен стабильным: он уже защищён private-API тестом, mix-offset policy вынесена в `MusicMixSettings`, а base/chase pause reason bookkeeping вынесен в `MusicPauseReasonState`; рискованный широкий audio-stack refactor лучше делать отдельной задачей с audio-regression focus.
...
- `CheckpointSceneSnapshot` больше не держит dynamic runtime restore policy напрямую: allowlist, restore metadata, parent resolution и scene instantiation вынесены в `CheckpointDynamicRestore`.
