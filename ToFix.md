...
Проект не выглядит разваленным. У него понятный entrypoint, явные autoload-и, рабочий локальный тестовый слой, Git LFS для ассетов, CI-проверки, `InteractionManager`, `SceneContext`, checkpoint-восстановление и набор контрактных тестов. После текущего remediation pass parser-only и полный suite проходили, полный suite содержит 82 теста.
...
- Scene NodePath contracts покрыты `test_scene_nodepath_contracts.gd`; STU hardcoded paths и dynamic door targets покрыты `test_stu_level_path_contracts.gd`, а configured `TriggerSetProperty` target/property wiring покрыт `test_trigger_set_property_contracts.gd`.
...
- `tests/cases/test_scene_nodepath_contracts.gd` валидирует active level/interactable scenes: unlocked/key doors require resolving targets, blockpost keeps `TouchArea` and `PassageBlocker/CollisionShape2D`, money interactables resolve money systems, and key exported visual/light/audio paths resolve. `tests/cases/test_trigger_set_property_contracts.gd` отдельно валидирует configured trigger target paths and property names.
